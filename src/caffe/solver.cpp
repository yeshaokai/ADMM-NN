...
void Solver<Dtype>::Step(int iters) {
  ...
  ComputeMask();
  KeepPrunedWeightsUnchanged();
  if(this->param_.clustering_phase()=="retrain"){
         Quantization();
	 Snapshot();
  }
  Initialization_get_interval();
...
template <typename Dtype>
void Solver<Dtype>::RestoreLabels(const char* state_file) {
  string state_filename(state_file);
  if (state_filename.size() >= 3 &&
      state_filename.compare(state_filename.size() - 3, 3, ".h5") == 0) {
    // NOT IMPLEMENTED
  } else {
    //RestoreLabelsFromBinaryProto(state_filename);
  }
}

template <typename Dtype>
void Solver<Dtype>::RestoreCentroids(const char* state_file) {
  string state_filename(state_file);
  if (state_filename.size() >= 3 &&
      state_filename.compare(state_filename.size() - 3, 3, ".h5") == 0) {
    // NOT IMPLEMENTED
  } else {
    RestoreCentroidsStateFromBinaryProto(state_filename);
  }
}

template <typename Dtype>
void Solver<Dtype>::RestoreCentroidsRange(const char* state_file) {
  string state_filename(state_file);
  if (state_filename.size() >= 3 &&
      state_filename.compare(state_filename.size() - 3, 3, ".h5") == 0) {
    // NOT IMPLEMENTED
  } else {
    RestoreCentroidsRangeStateFromBinaryProto(state_filename);
  }
}
